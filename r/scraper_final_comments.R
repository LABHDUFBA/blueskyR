...
# Função para obter respostas de uma postagem específica
get_post_replies <- function(post_uri, token) {
  ...
  return(content(response, as = "parsed")$thread$replies)  # Retorna as respostas (comentários) da postagem
}
...
get_all_replies_for_posts <- function(posts_df, token) {
  ...
  for (i in 1:nrow(posts_df)) {
    ...
    replies <- get_post_replies(post_uri, token)
    
    if (!is.null(replies)) {
      all_replies <- append(all_replies, replies)
    }
  }
...
# Função para converter as respostas em um data.frame
convert_replies_to_df <- function(replies_data) {
  replies_df <- do.call(rbind, lapply(replies_data, function(reply) {
    data.frame(
      ...
      reply_count = reply$post$replyCount,
      repost_count = reply$post$repostCount,
      like_count = reply$post$likeCount,
      indexed_at = reply$post$indexedAt,
      author_handle = reply$post$author$handle,
      author_display_name = reply$post$author$displayName,
...
# Exemplo de uso: buscar perfil, posts e respostas e salvar como CSV

# Substitua pelo nome de usuário no Bluesky sem @
username <- "marinahelenabr.com.br"  # Exemplo de nome de usuário
...
if (!is.null(profile_data) && length(posts_data) > 0) {
  ...
  replies_df <- convert_replies_to_df(replies_data)
...
# Exibir os nomes de usuário extraídos
print(usernames)

# Agora, você pode usar a variável 'usernames' para iterar sobre os perfis e obter os dados de cada um
for (username in usernames) {
  # Aqui você pode chamar as funções para coletar os dados do perfil, posts e comentários para cada usuário
  profile_data <- get_profile_data(username, token)
  posts_data <- get_all_posts(username, token)
  
  # Continue com o processamento de posts e respostas, como no código anterior
  if (!is.null(profile_data) && length(posts_data) > 0) {
    cat("Dados obtidos com sucesso para:", username, "\n")
